...
1. Skip if `remote_event_id` already in `menu_merge_remote_events` (existing dedupe). **If it is the echo of one of our own events** (matched via `_find_matching_local_merge` / `local_refs`), do not re-apply, but stamp its `server_seq` onto the affected rows and clear `pending_local`. If a row meanwhile carries a *higher* `assignment_seq` from someone else's event, re-apply that remote assignment and emit a "your resolution of X was superseded by {attribution}" notice (fixes I13's loser notification).
2. Otherwise, for each assignment in the event:
   ...
   - If the local row is `pending_local` → apply the remote assignment anyway (server order will judge; if our own event lands later on the server, its echo re-asserts our value everywhere, including here), but keep a record so step 1's ack-time comparison can notify.
   - Else UPDATE `menu_item_variants`, `order_items`, `order_item_addons` for that `order_item_id` to the event's target, set `is_verified`, set `assignment_seq = server_seq`.
...
4. **Snapshot endpoint**: `GET /desktop-analytics-sync/menu-assignments/snapshot?after_seq=&limit=` → `{"assignments": [...], "watermark_seq": <max MenuMergeEvent.id at snapshot time>, "next_page": ...}` (views/`menu_assignments.py`, wire in `urls.py`, bearer auth + scope like the others). Paged; deterministic order by `order_item_id`.
5. **Bootstrap rebuild** (demotion server-side, fixes the server half of **I6**): `menu_bootstrap_latest` for the menu portion is now *derived* — either regenerate `cluster_state` from `OrderItemAssignment` on read, or on a schedule. `menu_bootstrap_ingest` keeps accepting pushes (old clients) but stops overwriting the derived cluster state once Phase C4 clients are deployed (guard by payload flag `"snapshot_role": "seed_only"` sent by new clients; ops toggle `MENU_BOOTSTRAP_TRUST_CLIENT_CLUSTER_STATE=false`).
