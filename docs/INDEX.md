...
| [MENU_SYNC_ARCHITECTURE.md](./MENU_SYNC_ARCHITECTURE.md) | **As-built.** Assignment-based LWW sync + strict server-authoritative menu/customer commits. JSON seed runtime removal, catalog commits, derived-assignment flush, itemcode gates and state-scoped husk GC are complete. **§17 global menu groups:** revisions 1.6–1.8 are implemented but dormant. Revision 1.7 retains the identical-locator shared-POS policy; revision 1.8 adds mutually exclusive reviewed outlet aliases, private raw-itemcode observations, strict queue/preview/commit/timeout transport, operator UI, reconciliation diagnostics, many-to-one projection and unknown-locator quarantine (§17.6). Coordinated deployment may begin with both policies empty; Group 1 reconciliation and lifecycle activation remain an operator stop gate. Open follow-ups in `pending_task.md`. |
