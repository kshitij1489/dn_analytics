...
| **Global menu identity across restaurants (revision 1.9 active enrollment)** | [SYSTEM_CONTEXT.md](./SYSTEM_CONTEXT.md) → [MENU_SYNC_ARCHITECTURE.md](./MENU_SYNC_ARCHITECTURE.md) §17 → [central_server_analytics_app_api_contract.md](../contracts/central_server_analytics_app_api_contract.md) §25 |
...
| [MENU_SYNC_ARCHITECTURE.md](./MENU_SYNC_ARCHITECTURE.md) | **As-built.** Assignment-based LWW sync + strict server-authoritative menu/customer commits. JSON seed runtime removal, catalog commits, derived-assignment flush, itemcode gates and state-scoped husk GC are complete. **§17 global menu groups:** revision **1.9** active enrollment — membership plus ordinary Sync DB. POS locators are restaurant-scoped; unlinked rows stay in Unclustered Data Resolution; coverage does not gate capabilities. Shared-POS and POS Alias Review are removed. Open follow-ups in `pending_task.md`. |
...
| [SESSION_AND_ALL_STORES.md](./SESSION_AND_ALL_STORES.md) | Part A **reference** — cross-tool session conventions. Part B **as-built (2026-08-13)** — All Stores read-only federation: combination rules, global-ID menu identity when every included profile is aggregation-ready, physical-store-only actions, sequential Sync DB coordinator, runbook, performance budget. |
