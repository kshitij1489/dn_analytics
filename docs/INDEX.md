...
| **Global menu identity across restaurants (revision 1.10 + §25 catalog cutover)** | [SYSTEM_CONTEXT.md](./SYSTEM_CONTEXT.md) → [MENU_SYNC_ARCHITECTURE.md](./MENU_SYNC_ARCHITECTURE.md) §17 (as-built, including §17.8 cache epoch) → [central_server_analytics_app_api_contract.md](../contracts/central_server_analytics_app_api_contract.md) §25 |
...
Pulled from doc headers (refreshed 2026-08-17). See each doc for detail — do not treat this table as the source of truth. Refresh this table when a phase ships or a feature branch merges.
...
| [MENU_SYNC_ARCHITECTURE.md](./MENU_SYNC_ARCHITECTURE.md) | **As-built.** Assignment-based LWW sync + strict server-authoritative menu/customer commits. **§17 global menu groups:** revision **1.10** active enrollment plus the **catalog cutover** — central §25 is `backend/menu_catalog/`; desktop SQLite is a wire cache. Snapshot section cursors (including `rule_id`) are 32-char lowercase hex; event `after` stays integer; genesis watermark is 1/1. First desktop sync after this client bumps `global_menu_state.cache_epoch` and re-bootstraps snapshot+tail (no integer/`contract-rule:` translation). POS locators are restaurant-scoped, itemcode rules are parent-only, display-name aliases are retired, coverage is diagnostic-only. Group History defaults to human global changes, separates legacy/system (genesis is non-undoable backfill), and excludes `derived_assignment_v1`. Open follow-ups in `pending_task.md`. |
| [central_server_analytics_app_api_contract.md](../contracts/central_server_analytics_app_api_contract.md) | Shared wire contract. Baseline §5–15, §17 assignments, §19 menu commits, §21 customer commits, and the desktop §23–§24 revision-1.2 migration are **implemented**. §25 global menu is the **catalog cutover**: routes read/write `backend/menu_catalog/`; old server `global_menu_*` tables are unread until an explicit drop. Every scoped request carries an explicitly bound `X-Restaurant-ID`; raw analytics uses only `X-API-Key`. Retired wire surface is listed in §24. Desktop uses one SQLite profile per restaurant plus `analytics-control.db`; **All Stores is a local read-only federation and never reaches the wire** — every scoped request still names one physical restaurant. Twin-sync rule + checklist: [contracts/README.md](../contracts/README.md). |
...
| [SESSION_AND_ALL_STORES.md](./SESSION_AND_ALL_STORES.md) | Part A **reference** — cross-tool session conventions. Part B **as-built (2026-08-15)** — All Stores read-only federation: combination rules, global-only menu catalog/analytics with explicit omitted-row coverage, physical-store-only actions, sequential Sync DB coordinator, runbook, performance budget. |
