...
| **Global menu identity across restaurants (revision 1.10 active enrollment)** | [SYSTEM_CONTEXT.md](./SYSTEM_CONTEXT.md) → [MENU_SYNC_ARCHITECTURE.md](./MENU_SYNC_ARCHITECTURE.md) §17 → [central_server_analytics_app_api_contract.md](../contracts/central_server_analytics_app_api_contract.md) §25 |
...
| [MENU_SYNC_ARCHITECTURE.md](./MENU_SYNC_ARCHITECTURE.md) | **As-built.** Assignment-based LWW sync + strict server-authoritative menu/customer commits. **§17 global menu groups:** revision **1.10** active enrollment — POS locators are restaurant-scoped, human-confirmed itemcode rules are parent-only, display-name aliases are retired, and incomplete variant identity stays in Unclustered Data Resolution. Coverage does not gate capabilities. Group History defaults to human global changes, separates legacy/system activity, and excludes `derived_assignment_v1` bookkeeping from the central human-audit projection. Open follow-ups in `pending_task.md`. |
