...
| Global menu groups (shared canonical menu across restaurants that share a menu) | §25 | Central revision 1.9 implemented and tested — versioned snapshot/tail/assignment wire, complete semantic deltas, preview + strict commit/undo, four capabilities advertised together, restaurant-qualified POS rules by database check, and a separate editor permission. Enrolment is membership plus ordinary sync; there is no lifecycle rung and no operator bootstrap | Client rebuilt against revision 1.9: unlinked rows appear in Unclustered Data Resolution and are resolved through restaurant-scoped `global_locator.map` mutations |
...
**Deployment state (2026-08-13):** Group 1 owns the canonical catalog minted by the original cutover and advertises the four global-menu capabilities. Neither POS-policy setting was ever non-empty, so revision 1.9's removal changes no production behavior that was ever reachable; deploy both repositories together, then run migration `0032`.
