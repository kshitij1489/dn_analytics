...
| Global menu groups (shared canonical menu across restaurants that share a menu) | §25 | Central revision 1.10 implemented and tested — restaurant POS rules are qualified, human-confirmed itemcode rules are parent-only, alias authority is retired with archive+tombstone convergence, and coverage requires complete identity | Client rebuilt against revision 1.10: alias rules are inert/retired, itemcode targets are parent-only, and exact POS mappings complete variants |
