...
A group-wide locator rule requires `rule_scope: "group"` **and** `confirm_group_wide: true`, and only `itemcode` and `alias` may ask for it. A POS locator's rule is restaurant-scoped and always belongs to the selected restaurant. `restaurant_id` may be omitted or repeat `X-Restaurant-ID`; naming another member is refused. `applied.assignments_repointed` names only the selected restaurant; a peer that uses the same number for a different product is not in the set at all.
