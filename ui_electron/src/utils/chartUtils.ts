 ...
 * 4. Holiday Filtering
 * 5. Applying Metrics (Total, Average, Cumulative)
 */
...
/**
 * Maps a holiday date to its corresponding bucket key on the X-axis.
 * This ensures ReferenceLine x={...} matches an actual X-axis value.
 * 
 * @param holidayDate - The holiday date in "YYYY-MM-DD" format
 * @param timeBucket - 'Day', 'Week', or 'Month'
 * @returns The mapped date string that matches the X-axis bucket
 */
export function getHolidayXPosition(holidayDate: string, timeBucket: string): string {
    if (timeBucket === 'Day') {
        return holidayDate;
    }
    ...
    if (timeBucket === 'Week') {
        // Same logic as groupDataByTimeBucket: week ends on Sunday
        const dayOfWeek = date.getDay(); // 0 = Sunday
        const weekEnd = new Date(year, month - 1, day + (7 - dayOfWeek) % 7);
...
/**
 * Get holidays with their X-axis positions mapped for the given time bucket.
 * Use this instead of getVisibleHolidays when you need bucket-aware positioning.
 * 
 * @param rawData - The RAW (ungrouped) data array with date field
 * @param showHolidays - Whether to show holidays
 * @param timeBucket - 'Day', 'Week', or 'Month'
 * @returns Array of holidays with xPosition field added
 */
export function getHolidaysForChart(
    rawData: any[],
    showHolidays: boolean,
    timeBucket: string
): Array<{ date: string; name: string; xPosition: string }> {
    const holidays = getVisibleHolidays(rawData, showHolidays);
    return holidays.map(h => ({
        ...h,
        xPosition: getHolidayXPosition(h.date, timeBucket)
    }));
}


/**
 * Applies the selected metric ('Total', 'Average', 'Cumulative') to the grouped data.
 * Maps the result to the 'value' key for charting.
 */
export function applyMetric(data: any[], metric: string) {
    if (metric === 'Total') {
        return data.map(d => ({ ...d, value: d.revenue }));
    }
    if (metric === 'Average') {
        return data.map(d => ({ ...d, value: d.count > 0 ? d.revenue / d.count : 0 }));
    }
